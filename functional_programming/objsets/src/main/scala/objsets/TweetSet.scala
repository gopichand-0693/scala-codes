...
class Empty extends TweetSet:
  ...
  def mostRetweeted: Tweet = throw Error("No such thing for empty tweetset")
...
class NonEmpty( elem: Tweet, val left: TweetSet, val right: TweetSet) extends TweetSet:
  ...
  def descendingByRetweet: TweetList = new Cons(Tweet("Hello","world",2),Nil)
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet  =  TweetReader.allTweets.filter(x => x.text.contains(x))
  lazy val appleTweets: TweetSet  =  TweetReader.allTweets.filter(x => x.text.contains(x))
